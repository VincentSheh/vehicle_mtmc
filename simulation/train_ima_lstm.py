...
class EdgeIDSParallelEnv(PZooParallelEnv):
    ...
    def step(self, actions: Dict[str, int]):
        ...
        for _ in range(self.decision_interval):
            ...
            if len(self.env.history) >= self.n_edges:
                ...
                rew_per_agent = -(self.alpha * qsf + self.beta * lres + self.gamma_r * bcd)

                total_rew  += rew_per_agent
                total_lres += lres
                total_bcd  += bcd
                total_qsf  += qsf
                total_vio  += vio
    ...
    def _build_reward_per_agent(self) -> np.ndarray:
        ...
        return -(self.alpha * qoe_shortfall + self.beta * lambda_res + self.gamma_r * bcd)
