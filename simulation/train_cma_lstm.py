...
class InitRecurrentState(Transform):
    """
    Initializes recurrent states that TorchRL will store in the rollout.
    Shapes are SPEC-registered so TorchRL doesn't complain.

    Actor state (per agent): ("agents","recurrent_state_h/c") shape [B,E,1,H]
    Critic state (per env):  ("recurrent_state_h_v/c_v")      shape [B,1,H]
    """
    ...
    def _reset(self, td: TensorDictBase, td_reset: TensorDictBase, **kwargs) -> TensorDictBase:
        bs = tuple(td_reset.batch_size)  # [B]
        dev = td_reset.device
        B = bs[0] if len(bs) else 1
        E = self.n_edges
        H = self.hidden_dim

        td_reset.set(("agents", "recurrent_state_h"), torch.zeros((B, E, 1, H), device=dev))
        td_reset.set(("agents", "recurrent_state_c"), torch.zeros((B, E, 1, H), device=dev))
        td_reset.set("recurrent_state_h_v", torch.zeros((B, 1, H), device=dev))
        td_reset.set("recurrent_state_c_v", torch.zeros((B, 1, H), device=dev))
        td_reset.set(("agents","recurrent_state_h_out"), torch.zeros((B,E,1,H), device=dev))
        td_reset.set(("agents","recurrent_state_c_out"), torch.zeros((B,E,1,H), device=dev))
        td_reset.set("recurrent_state_h_v_out", torch.zeros((B,1,H), device=dev))
        td_reset.set("recurrent_state_c_v_out", torch.zeros((B,1,H), device=dev))                
        return td_reset

    def transform_observation_spec(self, observation_spec):
        bs = observation_spec.shape  # [B]
        dev = observation_spec.device
        E = int(observation_spec[("agents", "observation", "obs")].shape[-2])
        H = self.hidden_dim

        observation_spec[("agents","recurrent_state_h_out")] = UnboundedContinuousTensorSpec(shape=(*bs, E, 1, H), device=dev)
        observation_spec[("agents","recurrent_state_c_out")] = UnboundedContinuousTensorSpec(shape=(*bs, E, 1, H), device=dev)
        observation_spec["recurrent_state_h_v_out"] = UnboundedContinuousTensorSpec(shape=(*bs, 1, H), device=dev)
        observation_spec["recurrent_state_c_v_out"] = UnboundedContinuousTensorSpec(shape=(*bs, 1, H), device=dev)
        return observation_spec
...
class AgentRecurrentCore(nn.Module):
    def __init__(self, obs_dim: int, hidden_dim: int, device: str):
        super().__init__()
        self.hidden_dim = int(hidden_dim)
    ...
    def forward(self, td: TensorDictBase) -> TensorDictBase:
        obs = td.get(("agents","observation","obs"))   # step: [B,E,D], train: [B,T,E,D]
        is_init = td.get("is_init")                    # step: [B,1], train: [B,T,1] (often)

        step_mode = (obs.ndim == 3)
        if step_mode:
            obs = obs.unsqueeze(1)                     # [B,1,E,D]
        if is_init.ndim == 2:
        ...
        # ---- pull recurrent states from td ----
        h = td.get(("agents","recurrent_state_h"))      # step: [B,E,1,H] or train: [B,T,E,1,H]
        ...
        h_be = h0.reshape(B * E, 1, H).contiguous()     # [B*E,1,H]
        c_be = c0.reshape(B * E, 1, H).contiguous()

        td_be = TensorDict(
            {
                "recurrent_state_h": h_be,
                "recurrent_state_c": c_be,
            },
            batch_size=[B * E],
            device=feats_be.device,
        )

        # ---- unroll manually, feed 2D features each step ----
        out = []
        for t in range(T):
            td_be.set("is_init", is_init_be[:, t])      # [B*E,1]
            td_be.set("features_t", feats_be[:, t])     # [B*E,H]  ✅ 2D input to LSTM
            self.lstm(td_be)
            out.append(td_be.get("features_t"))         # [B*E,H]

        feats_be2 = torch.stack(out, dim=1)             # [B*E,T,H]
        feats_btEH = feats_be2.reshape(B, E, T, H).transpose(1, 2).contiguous()  # [B,T,E,H]

        # write features for head
        # write features
        if step_mode:
        ...
        # after the unroll loop
        h_next_be = td_be.get("recurrent_state_h")  # updated by LSTMModule
        c_next_be = td_be.get("recurrent_state_c")

        if step_mode:
            td.set(("agents","recurrent_state_h_out"), h_next_be.reshape(B, E, 1, H))
            td.set(("agents","recurrent_state_c_out"), c_next_be.reshape(B, E, 1, H))
...
def build_env_stack(env_cfg: dict, train_cfg: dict, cfg_path: str, num_envs: int):
    ...
    transforms: List[Transform] = [
        ...
        InitRecurrentState(n_edges=n_edges, hidden_dim=hidden_dim),
...
def train(
    ...
    # --- actor ---
    actor_core = AgentRecurrentCore(obs_dim=obs_dim, hidden_dim=hidden_dim, device=device)
