...
def train(env_cfg_path="./configs/simulation_0.yaml", train_cfg_path="./configs/train.yaml", device="cuda"):
    ...
    # Write modified env_cfg to a temporary file for workers to read
    with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as tmp:
        yaml.dump(env_cfg, tmp)
        env_cfg_path = tmp.name

    # Append levels to exp_name to ensure distinct checkpoint directories
    ...
    try:
        ...
        loss = ClipPPOLoss(
            ...
            entropy_bonus=True,
            entropy_coeff=train_cfg["loss"]["entropy_coeff"],
