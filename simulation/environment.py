...
def build_env_from_cfg(cfg: dict):
    ...
    for area_cfg in cfg["edge_areas"]:
        ...
        if not area_users_cfg and global_user_cfg:
            # Resolve target_avg from level if present
            target_avg = global_user_cfg.get("target_avg")
            if "target_avg_level" in global_user_cfg:
                lvl = global_user_cfg.get("level", global_user_cfg.get("synthetic", {}).get("level", "default"))
                target_avg = global_user_cfg["target_avg_level"].get(lvl, global_user_cfg["target_avg_level"]["default"])
            ...
            for u in area_users_cfg:
                ...
                if "target_avg_level" in base_cfg:
                    lvl = base_cfg.get("level", base_cfg.get("synthetic", {}).get("level", "default"))
                    target_avg = base_cfg["target_avg_level"].get(lvl, base_cfg["target_avg_level"]["default"])
