...
class Attacker:
    ...
    def _init_start(self):
        ...
        # Single-edge enforce min start delay of 500.
...
class User:
    ...
    def __init__(
        ...
        arrival_col: str = "raw_count",
        random_slice: bool = True,
        target_avg: Optional[Union[float, List[float], Tuple[float, float]]] = None,
    ):
        ...
        # Handle target_avg (can be a single value or a range [min, max])
        if isinstance(target_avg, (list, tuple)) and len(target_avg) == 2:
            self.target_range = (float(target_avg[0]), float(target_avg[1]))
            # Maintain a singular target_avg for backward compatibility/logging
            self.target_avg = (self.target_range[0] + self.target_range[1]) / 2.0
        else:
            self.target_avg = float(target_avg) if target_avg is not None else None
    ...
    def _init_from_trace(self):
        n = self.full_trace.shape[0]
        if n == 0:
            self._req = np.zeros(self.t_max, dtype=np.int32)
            return

        start_idx = self._sample_start_index()
        
        # Use circular indexing to wrap around the dataset
        indices = np.arange(start_idx, start_idx + self.t_max)
        chunk = np.take(self.full_trace, indices, mode='wrap')
        
        if self.target_range is not None:
            # Match gt_to_trace.py: robust percentile scaling based on FULL trace
            mu_min, mu_max = self.target_range
            p2, p98 = np.percentile(self.full_trace, [2, 98])
            chunk = mu_min + (chunk - p2) / (p98 - p2 + 1e-9) * (mu_max - mu_min)
            chunk = np.clip(chunk, mu_min * 0.8, mu_max * 1.2)
        elif self.target_avg is not None:
            # Fallback proportional scaling if only a single target_avg is provided
        ...
        self.slice_end = start_idx + self.t_max
        self.df = None
