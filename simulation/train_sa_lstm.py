...
class TorchRLEnvWrapper(EnvBase):
    ...
    def _step(self, tensordict: TensorDict) -> TensorDict:
        ...
        # The giving service releases CPUs immediately; the receiving service gains only after the transition.
        # scale-up   (delta > 0): VA gives immediately → overhead_va = delta  (VA runs at budget-target)
        # scale-down (delta < 0): IDS gives immediately → overhead_ids = |delta| (IDS runs at target)
